// 渲染商家组件
<template>
  <section class="index-contaniner" @click="$router.push('/shop')">
    <div class="index-shopInfo">
      <!-- 左侧图片 -->
      <div class="img">
        <img :src="restaurant.image_path" alt>
      </div>
      <!-- 右侧内容 -->
      <div class="info">
        <div class="index-main">
          <!-- 名字 -->
          <div class="index-shopName">
            <i v-if="restaurant.is_premium">品牌</i>
            <span>{{ restaurant.name }}</span>
          </div>
          <!-- 星级 -->
          <div class="index-rateWrap">
            <div>
              <div class="rating-gray">
                <hx-star :rating="restaurant.rating"></hx-star>
              </div>
              <span class="rate">{{ restaurant.rating }}</span>
              <span>月售{{ restaurant.recent_order_num }}单</span>
            </div>
            <div class="delivery" v-if="restaurant.delivery_mode">
              <span class="icon-hollew">{{ restaurant.delivery_mode.text }}</span>
            </div>
          </div>
          <div class="index-moneyLimit">
            <div>
              <span>￥{{ restaurant.float_minimum_order_amount }}起送 | </span>
              <span>配送费￥{{ restaurant.float_delivery_fee }}</span>
            </div>
            <div class="index-distanceWrap">
              <span>{{ (restaurant.distance/1000).toFixed(2) }}km | </span>
              <span>{{ restaurant.order_lead_time }}分钟</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
import star from '../components/star'
export default {
  props: {
    restaurant: Object
  },
  components: {
      'hx-star': star
  }
};
</script>

<style lang="scss" scoped>
.index-contaniner {
  background-color: #fff;
  color: #666;
  padding: 4vw 0;
  border-bottom: 0.133333vw solid #eee;
  .index-shopInfo {
    display: flex;
    justify-content: flex-start;
    padding: 0 2.666667vw;
    align-items: stretch;
    .img {
      width: 17.333333vw;
      height: 17.333333vw;
      img {
        display: block;
        width: 100%;
        height: 100%;
        box-sizing: border-box;
        border: 0.133333vw solid rgba(0, 0, 0, 0.08);
        border-radius: 0.533333vw;
      }
    }

    .info {
    flex: 1;
      .index-main {
        display: flex;
        justify-content: space-between;
        overflow: hidden;
        flex-direction: column;
        padding-left: 2.666667vw;
        font-size: 0.2rem;
        flex-grow: 1;
        .index-shopName {
          align-items: center;
          color: #333;
          font-weight: 700;
          font-size: 0.9rem;
          i {
            background: #ffe800;
            margin-right: 1.333333vw;
            padding: 0.266667vw 0.666667vw;
            text-align: center;
            white-space: nowrap;
            font-size: 0.6rem;
            font-style: normal;
          }
          span {
            overflow: hidden;
            text-overflow: ellipsis;
            white-space: nowrap;
          }
        }
        .index-rateWrap {
          display: flex;
          align-items: center;
          overflow: hidden;
          justify-content: space-between;
          margin-top: 10px;
          font-size: 0.7rem;
          .delivery {
            display: flex;
            align-items: center;
            font-size: 0.6rem;
            margin-left: 1.066667vw;
            .icon-hollew {
              color: #fff;
              background-color: #2395ff;
              padding: 2px;
              box-sizing: border-box;
            }
          }
          .rate {
            margin-right: 1.066667vw;
          }
        }
        .index-moneyLimit {
          width: 100%;
          display: flex;
          justify-content: space-between;
          margin-top: 10px;
          font-size: 0.7rem;
          .index-distanceWrap {
            color: #999;
          }
        }
      }
    }
  }
}
</style>
