  ...
  <div v-if="shopInfo" class="goods">
        ...
        <ul>
          <li v-for="(item, index) in recommend.items" :key="index">
            ...
            <div class="recommend-food-price">
              ...
              <hx-catControll :food="item"></hx-catControll>
            </div>
          </li>
        </ul>
      </div>
    ...
    <div class="menuview">
      ...
      <div class="menu-wrapper" ref="menuScroll">
        <ul>
          <li v-for="(val, index) in shopInfo.menu" :key="index" @click="selectMenu(index)" :class="{'current': currentIndex == index}">
            <img :src="val.icon_url" alt v-if="val.icon_url">
            <span>{{ val.name }}</span>
          </li>
        </ul>
      </div>
      ...
      <div class="foods-wrapper" ref="foodScroll">
        <ul>
          <li v-for="(val, index) in shopInfo.menu" :key="index" class="food-list-hot" >
            <div class="category-title">
              <strong>{{ val.name }}</strong>
              <span>{{ val.description }}</span>
            </div>
            <div class="fooddetails" v-for="(food, i) in val.foods" :key="i">
              <!-- 左图片 -->
              ...
              <section class="fooddetails-info">
                ...
                <div class="fooddetails-price">
                  ...
                  <hx-catControll :food="food"></hx-catControll>
                </div>
              </section>
            </div>
          </li>
...
<script>
import catControll from "../../components/shops/cartControll";
...
export default {
  ...
  methods: {
    ...
    selectMenu(index) {
      // console.log(index);
      // 根据索引位找到-右侧对应区域
      this.foodScroll.scrollToElement(this.$refs.foodScroll.children[0].children[index], 250)
    },
    height () {
      this.listHeight.push(0);
      let height = 0;
      for(let i = 0; i < this.shopInfo.menu.length - 1; i ++){
        let hg = this.$refs.foodScroll.children[0].children[i].offsetHeight;
        height += hg;
  ...
  components: {
    "hx-catControll": catControll
  },
  computed: {
    currentIndex () {
      for(let i =0; i < this.listHeight.length; i ++){
        let start = this.listHeight[i];
        let end = this.listHeight[i + i];
