<template>
  <div class="shop" v-if="shopInfo">
    ...
    <!-- 商家信息 -->
    <div class="index-rst">
      <div class="rst-name" @click="show=true">
        <span>{{ shopInfo.rst.name }}</span>
        <i class="fa fa-caret-right"></i>
      </div>
      <!-- 弹窗信息 -->
      <hx-info :rst="shopInfo.rst" :show="show" @changeshow="changeshow"></hx-info>
      <!-- 信息 -->
      <div class="rst-order">
        <span>评分{{ shopInfo.rst.recent_order_num }}</span>
        <span>月售{{ shopInfo.rst.recent_order_num }}单</span>
        <span>蜂鸟专送约{{ shopInfo.rst.order_lead_time }}分钟</span>
      </div>
      <!-- 优惠 -->
      <hx-activite :activities="shopInfo.rst.activities"></hx-activite>
...
<script>
import info from "../../components/shops/info";
import activite from "../../components/shops/activite"
